    ...
    static show(options) {
        ...
        if (type == 'object' && typeof component == 'object')
            template = `<div class="w680" v-if="show">
                            <el-dialog :style="style" :title="title" v-model="show" :close-on-click-modal="false" :size="size" :before-close="beforeClose" @close="close">
                                <child v-on:click="click" :data="data"></child>
                            </el-dialog>
                        </div>`;
        else if (type == 'html' && typeof component == 'string')
            template = `<div class="w680" v-if="show">
                            <el-dialog :title="title" v-model="show" :close-on-click-modal="false" :size="size" @close="close">${component}</el-dialog>
                        </div>`;
        else if (type == 'componenttag' && typeof component == 'string')
            template = `<div class="w680" v-if="show">
                            <el-dialog :title="title" v-model="show" :close-on-click-modal="false" :size="size" @close="close">
            ...
            template = `<div class="w680 dialog-h400" v-if="show">
                            <el-dialog  :title="title" v-model="show" :close-on-click-modal="false" :size="size">
